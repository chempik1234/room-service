...
3. Мы получаем обновления, которые вызывают и другие клиенты, потому что приложение может быть отмасштабировано,
   может иметь несколько соединений, а также у клиента может быть возможность связываться с сервисом напрямую: там
   необходима рассылка событий.
